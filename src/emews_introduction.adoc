...
. *Install Conda*
+
The EMEWS binary install is a conda environment, and requires an https://www.anaconda.com/download[anaconda]
...
you do not have an existing anaconda or miniconda installation.
+
...
[source, bash]
----
$ source <your_conda_install>/bin/activate
----
For example,
+
[source, bash]
...
----
The install script (`install_emews.sh`) takes two
arguments:
...
+
For example, this will install the EMEWS environment with Python 3.11 and create the EMEWS DB database
in the `~/Documents/db/emews_db` directory. 
[source, bash]
----
...
See <<_installing_emews>> for additional details about the install script,
and other kinds of installs.
