 ...
 * `data` - data required by the model and algorithm (e.g., input data).
...
Python model exploration (ME) algorithm.  EQ/Py is a Swift-t extension that adds two blocking queues through which a Swift script and a Python code algorithm can coordinate execution. The following is the EMEWS workflow structure with EQ/Py,
...
The eqpy template installs this extension and adds user customizable code that allows model runs and exploration to be controlled from a python code algorithm using the two queues. By default the coordinated execution of the two follows this pattern:
...
Steps 5 - 8 are repeated until some stopping condition (e.g. algorithm convergence or some maximum number of iterations) is reached.
...
The eqr template works much the same as the eqpy template, but for model exploration algorithms written in R. It generates files and code for model exploration using the EQ/R EMEWS extension. The EQ/R extension adds two blocking queues through which a Swift/T script and a R code algorithm can coordinate execution. The following is the EMEWS workflow structure with EQ/R,
...
The eqr template installs this extension and adds user customizable code that allows model runs and exploration to be controlled from a R code algorithm using the two queues. By default the coordinated execution of the two uses the following pattern (identical to that of the eqpy template).
...
IMPORTANT: The extension needs to be compiled before it can be used. See `{eqr_dir}/src/README.md` for compilation instructions.
