[[creating_emews_projects, Creating EMEWS Projects]]
...
EMEWS Creator also includes an `init_db` command that creates a fuly initialized EQSQL database
in a user specified directory. `init_db` creates a postgresql database cluster in a specified path,
then creates a user and databasein that cluster, and finally populates that datbase with the required
