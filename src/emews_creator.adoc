...
submission), and swift-t scheduled worker pool submission#
...
the workflow_name, can model_name configuration parameters. Here the workflow name is `eqsql`, and the model name is `my model`. If Python or R is specified in the `me_language` parameter, then an example
ME algorithm and configuration files are created. Here, the me language is Python, the me file name is `algo`, and the me cfg file name is `algo_cfg`. 
...
eqsql database. Typically, this should not need to be edited by a user.
* `ext/EQ-SQL/eqsql_swift.py` - Python code used by worker pools to retrieve tasks and report results to the eqsql database. Typically, this should not need to be edited by a user.

These files (excluding those in `ext/EQ-SQL`) contain lines or sections marked with *TODO* where that line or section needs
to be edited to customize the file for your model and workflow. See TODO*<<uc4>>* for a fully fleshed out
sweep workflow created using emews creator. We will look more closely at relevant parts of these files
below.

#TODO: run_eqsql_worker_pool.sh link#
// https://github.com/jozik/emews_next_gen_tutorial_tests/blob/2843e5d305c93e38f56988c75e1a723e73b30d8d/code/eqa l_project/swift/run_eqsql_worker_pool.sh#L1[`*run_eqsql_worker_pool.sh*`,window=eqsql_worker_pool.sh,pts="noopener,nofollow"]
...
those values so that they are available to swift when creating the job submission
script. These are set from values defined in the workflow configuration file
(i.e., swift/cfgs/eqsql_worker_pool.cfg). See the discussion of that file below.
#TODO: make link#
...
<3> If there are errors when running R code in workflows, this section
can be edited appropriately and uncommented.
<4> Set an environment variable for the EQ/SQL swift extension. This is
used internally by the workflow, and should not be edited.
<5> Set an environment directory for the EMEWS swift extensions. This is
used internally by the workflow, and should not be edited.
...
edited there if necessary. This variables will be further explained
in the configuration file section #TODO: make link#.
...
the worker pools stdout and stderr to a file when running on a non-scheduled resource.
...
The launch script passes some arguments to the swift script via the command line.
...
id using that. These arguments are all set via the configuration file and will be discussed
in more detail there. #TODO: make link#.
...
<2> Define a variable containing the name of the swift workflow script to
execute.
<3> Call swift-t passing it the relevant variables and the path of the swift script to be executed. At this point the script is compiled and then either executed immediately or scheduled for execution depending on the value of the MACHINE variable.
<4> The `-e` agument to swift-t adds the specified variable to the script execution environment. On some HPC machines,
the login environment is separate from the compute environment. Consequently, variables defined in the login environment
that are referenced in the swift script when it executes in the compute environment need to be made available for the
script to work correctly. The `-e` argument does this, adding the specified variables to the compute environment.
...
#TODO: eqsql_worker_pool.swift link#
// https://github.com/jozik/emews_next_gen_tutorial_tests/blob/2843e5d305c93e38f56988c75e1a723e73b30d8d/code/eqa l_project/swift/run_eqsql_worker_pool.sh#L1[`*run_eqsql_worker_pool.sh*`,window=eqsql_worker_pool.sh,pts="noopener,nofollow"]
...
----
<1> Set emews_root and turbine_output from the EMEWS_PROJECT_ROOT and TURBINE_OUTPUT environment variables. These were exported in the #TODO: make link# run_eqsql_worker_pool.sh script.
<2> Set the TASK_TYPE, BATCH_SIZE, BATCH_THRESHOLD, and WORKER_POOL_ID variables. These are used
by the swift script when fetchting tasks and reporting their output.
...
The script executions begins by calling the start function in which batch querying
for tasks from the database is initialized. 
...
[IMPORTANT]
====
Batch querying allows a worker pool to request up to `BATCH_SIZE` number of tasks to consume at a time, while accounting for the number of tasks a worker pool already has obtained but have not completed. So, for example, if a worker pool is configured to possess 33 tasks at a time, if it owns 30 uncompleted tasks when querying the output queue, it will only obtain 3 additional tasks. This can be tweaked using `BATCH_THRESHOLD` value that specifies how large the deficit between requested tasks and owned tasks must be before more tasks are obtained.
Querying for tasks in this way allows a worker pool to tune its query to the number of available workers such that all its workers are busy while equitably sharing work among multiple worker pools, when those exist.
====
...
----
<1> Query for an array tasks to execute. The tasks are formatted as `message`-s. Each message
has an integer task_id, a string msg_type, and string payload. The task_id is a unique
identifier for the task, the msg_type specifies the type of message: a `status` or `work`.
The payload consists of either a status specification, or parameters for the task to be performed in JSON format.
<2> If the first of the messsage types is `status` then exit the loop with the appropriate output.
<3> If the first of the message types is not `status`, then execute the array of task messages.

The run function executes the tasks concurrently in a swift `foreach` loop, and reports the results back.
...
`run_task` executes the specifed number of trials for an individual task (a model run), running the task with the same parameters and varying a random seed parameter. All the trials will run in an `instance_N` directory
...
trial number, instance direcdtory, and instance_id. The result from each trail
...
----
<1> Creates swift file objects to capture the standard out and standard error from the `run_task_app`
function.
...
`run_task_app` calls `scripts/run_my_model.sh` (#TODO: make link#) passing it the task payload,
...
The final two functions are the result calcualtions. These need to be completed by the user
...
#TODO: link to eqsql_worker_pool.cfg#
// https://github.com/jozik/emews_next_gen_tutorial_tests/blob/2843e5d305c93e38f56988c75e1a723e73b30d8d/code/emews_project/swift/cfgs/sweep_workflow.cfg#L1[`*eqsql_worker_pool.cfg*`,window=UC1,pts="noopener,nofollow"]
`eqsql_worker_pool.cfg` contains configuration variables for running the worker pool. It is sourced
by the submission script #TODO: make link# `run_eqsql_worker_pool.sh` to retrieve the database connection parameters, the HPC scheduler parameters, and the other variables required by the workflow.
...
The final set of parameters set the number of trials, the worker pool id, the task type 
...
#TODO: link to run_my_model.sh#
// https://github.com/jozik/emews_next_gen_tutorial_tests/blob/2843e5d305c93e38f56988c75e1a723e73b30d8d/code/emews_project/swift/sweep_workflow.swift#L1[`*run_my_model.sh*`,window=UC1,pts="noopener,nofollow"]

`run_my_model.sh` is called by `eqsql_worker_pool.swift` to execute the model as if the model has been run from the command line. The script passed model input parameters (the payload string), the output file for the
model to use, the trial number for this set of parameters, the emews root directory location, and the instance directory that was created in `eqsql_worker_pool.swift`. `run_my_model.sh` in clues two important *TODOs*. 
Y ou will need to update the `MODEL_CMD` variable to specify the model executable, and the `arg_array` to
...
#TODO: link to algo.py#
// https://github.com/jozik/emews_next_gen_tutorial_tests/blob/2843e5d305c93e38f56988c75e1a723e73b30d8d/code/emews_project/swift/cfgs/sweep_workflow.cfg#L1[`*algo.py*`,window=UC1,pts="noopener,nofollow"]

If the `me_language` argument to the `esql` command is `python`, then an example Python ME and ME configuration file will be produced. The ME can be run from the command line as follows:
...
* `exp_id` - an identifier for the current run the workflow (e.g., epi_model_experiment_3).
* `config_file` - the path to ME configuration file (e.g., `algo_cfg.yaml`)

The example ME contains two functions and a `if __name__ == 'main'` section. 
...
def run(exp_id: str, params: Dict):    <1>
  ...
  pool = worker_pool.start_local_pool(params['worker_pool_id'], params['pool_launch_script'],    <5>
                                      exp_id, pool_params)
...
These will be explained in more detail in the #TODO: make link# ME configuration file
discussion.
<2> Start the database located at the `db_path` parameter.
<3> Initialize a task queue for submitting tasks to the database output queue.
...
Emews Creator also includes an `init_db` command that does not create a workflow, but rather creates 
